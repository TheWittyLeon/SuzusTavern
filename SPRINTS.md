...
|------|-------|--------|-------|
| 2026-08-19 | `94a21f0` | **DEPLOYED + PROD-VERIFIED** | **PROD NIGHT — FULL QUEUE, four artifacts across three repos, Leon-authorized ("Tonight if quiet — full queue").** Tavern half: `fix/tav-dnd-proxy-bearer-removal` @ `813fc88` (cookie-wins — client-supplied `Authorization` header no longer overrides the `st_access` cookie in the dnd + narration BFF proxies; Kuro-Sec CLEAR, no conditions). **Caught a real divergence before it became a silent revert**: this branch and the same-day daylight-window deploy above (`cf8f44c`) shared a common ancestor but were NOT linear — both had independently authored the path-traversal fix under different SHAs (`9603257` vs the deployed `cf8f44c`). A raw tree rsync of the branch would have reverted `cf8f44c`'s already-deployed work. Reconciled by cherry-picking only the two unique commits (`478511a` cookie-wins + `813fc88` colon-boundary regression test) onto the real deployed baseline; verified clean auto-merge (zero conflicts) and byte-identical final content against the source branch's own diff before re-gating. Gates on the reconciled tip: tsc clean, eslint 0 errors (1 pre-existing unrelated warning), jest 203 suites/2937 tests green, `next build` clean. Secret scan clean. Quiet-check re-confirmed at the deploy moment (0 non-heartbeat log lines across engine/api/auth, 0 DB writes in 10min). Rollback staged first: image `nekonova-tavern:rollback-prodnight-20260819T200822Z` + tree tarball `/home/leon/backups/suzustavern-pre-prodnight-20260819T200822Z.tar.gz`. Deploy: rsync overlay (no `--delete`) → `docker compose -f docker-compose.tavern.yml up -d --build`; healthy, RestartCount 0. **Live-verified with a positive+negative control pair** via the real prod test account (`suzu-prod-tester`, real BFF login): valid cookie + a bogus forged client `Authorization` header → `200` with the cookie-user's own character list (cookie wins, forged bearer ignored); the same bogus bearer alone with no cookie → `401 actor_required` (no silent anonymous forward). Today's earlier traversal-probe regression check still `400 invalid_path`. Set-Cookie headers confirmed `HttpOnly; SameSite=lax`, no `Secure` (correct for this plain-HTTP homelab deploy). Sibling artifacts this same window: NekoNova-DnDEngine `fix/engine-bootguard-and-persisted-20260819` @ `91bc27b` (actor-enforcement boot-guard, enforced gate re-measured 187/1/exit 0 on this exact SHA) and ProjectNekoNova `feature/dm-latency-3fix` + `fix/dm-override-contract-comment` (Ollama keep_alive latency fix, rebuilt BOTH api and bot per blast-radius — bot imports `core/` directly) — see those repos' own closure notes / vault Current Status for full detail. `main` FF'd `05cc44c`→`94a21f0` (strict, 2 commits — rebased mid-close onto a concurrent docs-only push, re-verified byte-identical post-rebase before pushing; no force-push). |
| 2026-08-19 | `cf8f44c` | **DEPLOYED + PROD-VERIFIED** | **DAYLIGHT MINI-WINDOW — 2 reviewed-clean fixes, Tavern-ONLY (no NekoNova/engine).** `fix/tav-dm-override-reason-pairing` (Kage-CR APPROVED WITH COMMENTS, Miko PASS) closed the live-broken DM-override UX (DmOverrideModal branched on the retired `not_dm` reason; now a reason-map with an honest `combat_not_found` message). `fix/tav-dnd-proxy-path-traversal` (Kuro-Sec CLEAR, Miko PASS) closed TAV-DND-PROXY-PATH-TRAVERSAL (P0). Branches were cut from different bases — built a clean integration (`integration/tav-daylight-mini-window-20260819`, worktree, main tree untouched — it holds unrelated WIP) by cherry-picking both onto deployed tip `f75ff79`; the traversal commit's base predated 6 intervening main commits incl. one touching the same file (F2 `upstream_non_json` guard) — auto-merged clean, zero conflicts, verified byte-identical to each source branch's own diff. Gates on the integrated tree: tsc clean, eslint 0 errors (1 pre-existing unrelated warning, untouched file), jest 203 suites/2931 tests green, `next build` clean (env vars matched prod compose build args). Secret scan clean, no `.env*` in diff. Quiet-check: 90min zero real traffic (engine/Tavern/auth logs — only idle poll + healthchecks), 0 `msm.campaigns` activity in 2h, 0 generation jobs in 28 days; re-confirmed at the deploy moment. Rollback staged first: image `nekonova-tavern:rollback-daylight-20260819` (`7e6345b6`) + tree tarball `/home/leon/backups/suzustavern-pre-daylight-20260819.tar.gz`. Deploy: rsync overlay (no `--delete`) → `docker compose -f docker-compose.tavern.yml up -d --build`; healthy, RestartCount 0. **Live-verified**: encoded `..%2f..%2f..%2fapi%2falerts%2ftest` (GET+POST, the exact unauthenticated `/api/alerts/test` OBS-overlay vector) → `400 invalid_path`, upstream never called; legit proxy pass-through unbroken (`/api/dnd/systems` → real `401 actor_required`); real browser session as suzu-prod-tester (login→dashboard→play, an existing dormant campaign with stored active-combat state) — DM Override modal opened cleanly with the new copy, 0 console errors end-to-end, closed without submitting (no state mutated). `main` FF'd `f75ff79`→`cf8f44c` (strict, 3 commits, pushed, no force-push). |
| 2026-08-17 | `7bfa34c` | **DEPLOYED + PROD-VERIFIED** | **PROD NIGHT — the Tavern sprint branch is LIVE ON PROD, and `main` now equals it.** Deployed to `nekonova-tavern` on nekonova-aux (10.69.69.127:3000) from `sprint/tavern-20260811` @ `7bfa34c` — **NOT `main`**, which was 5 commits behind and lacked the leave-campaign UI entirely (caught by the runbook's step-0b unmerged-branch enumeration; deploying "current tip" would have shipped a Tavern without the feature the engine fix exists to enable). 77 files overlaid, aux tree then **byte-identical to the artifact across all 205 tracked files**; rebuilt on aux, healthy, `RestartCount=0`, image `7e6345b6` (was `4522c11b`); 33 month-old AppleDouble `._*` files swept (`COPYFILE_DISABLE=1` created none). Rollback image `nekonova-tavern-rollback:prodnight-20260817T025247Z` + tree tarball retained. **The leave-campaign chain is live end-to-end for the first time** (engine `e4f3957` + NekoNova proxy hop `cc76daa` + this Tavern), proven with a three-probe shape: the new BFF hop and a pre-existing sibling both return byte-identical `401 actor_required`, while a genuinely absent route now returns a clean `404 upstream_non_json` instead of the old blind 500 — **that third probe is what proved the F2 `upstream.json()` guard on the live system**, since a success-path test never exercises it. `main` FF'd `aae7a43`→`7bfa34c` (strict, 5 commits, pushed, no force-push). C3 outcome lines remain **inert by design** — the engine renamed/nested the field, and the Tavern type-guards and early-returns, so it renders nothing rather than crashing. Full record: vault `planning/2026-08-17 PROD NIGHT — COMPLETED.md`. |
...
| 2026-08-15 | WF-H + WF-I + WF-J + hotfix windows #1/#2 + back-merge | engine 75c35d3, NOT deployed — rides prod night; BOTH hotfixes LIVE ON PROD (main b49b512) | WF-H extended the always-on two-part guard (end/next-act) and fixed a raw-vs-normalized username compare that 404'd legitimate mixed-case self-clears. WF-I converted 26 routes to always-on require_genuine_actor + unconditional guard_*, ruled by Leon as a CLASS after the detector proved the enumeration was 16+, not the literal-grep 8; armed re-run 2548 passed with the guard PROVEN armed by differential positive control; Kage CLEARED WITH COMMENTS. WF-J delivered the threat model. TWO PROD HOTFIX WINDOWS EXECUTED: #1 channel/path guard (authz.channel_mismatch WARN), #2 re-entry authz closing a CRITICAL campaign-takeover plus a HIGH DM-identity oracle — 13/13 prod smoke with positive controls first, DB-level proof that no attacker row was ever written, both refusals byte-identical. Residual sweep: zero stale rows on prod full-population (positive control first), dev best-effort across 15 identities — no purge, no notify. WF-K(b) back-merged main into the sprint branch (75c35d3, both hotfixes in ONE merge, identical commits never twins): Kuro-Sec CLEAR with both controls byte-identical to prod, Kage CHANGES REQUESTED on documentation only — the resolution itself confirmed correct. |
| 2026-08-13 | WF-G BYPASSRLS guard sweep | ea17929, NOT deployed — rides prod night | bind (incl. CLEAR branch) + delete moved to the always-on two-part guard, mutation-proof; AUTHZ_CONTRACT known-open list restored and corrected. SCOPE CORRECTION: the feared Twitch-bot risk was in the WRONG PLACE — the bot calls neither swept route; the single point of failure is the NekoNova Flask proxy, the only actor-header producer for web traffic. DND_SERVICE_TOKEN revoked on these routes. Two further routes (end/next-act) and the CLEAR 200-vs-404 footgun surfaced and escalated; Leon ruled both, WF-H in flight. Dev debris purge executed (16 rows, survivors asserted). |
